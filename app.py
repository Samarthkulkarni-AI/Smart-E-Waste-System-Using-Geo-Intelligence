...
from geoalchemy2.functions import ST_AsGeoJSON, ST_Point, ST_DWithin, ST_GeogFromWKB 
...
def inject_active_profile():                        # This function now provides 'get_active_profile' AND 'unread_notifications' to all of your templates, all the time.
...
def write_completed_request(req):
    csv_path = "completed_requests.csv"
    file_exists = os.path.isfile(csv_path)

    disposer = req.disposer_profile.full_name if req.disposer_profile else "Unknown"
    collector = req.collector_profile.full_name if req.collector_profile else "Unknown"
...
def mark_completed(request_id):
    ...
    # Automatically write to CSV
    write_completed_request(req)
...
def get_pending_requests_data():
    ...
    # Query the database for all pending requests that HAVE a location using ST_AsGeoJSON to convert the PostGIS 'location' point into a format (GeoJSON) that Leaflet can understand.
    ...
    features = []
    for req in requests:
        location_json = json.loads(req[3]) # Parses the GeoJSON string
        features.append({
            "type": "Feature",
...
def create_profile():
    if request.method == 'POST':
        ...
        # Restrict to only 1 disposer and 1 collector per user
        existing_profiles = Profile.query.filter_by(user_id=current_user.id).all()

         # Check for existing disposer
        if any(p.role == 'disposer' for p in existing_profiles) and role == 'disposer':
            flash('⚠️ You already have a disposer profile. Only one is allowed.', 'warning')
            return redirect(url_for('create_profile'))

        # Check for existing collector
        if any(p.role == 'collector' for p in existing_profiles) and role == 'collector':
            flash('⚠️ You already have a collector profile. Only one is allowed.', 'warning')
            return redirect(url_for('create_profile'))
...
if __name__ == '__main__':
    with app.app_context():
        # db.drop_all() # IMPORTANT: If needed to drop all tables, you must temporarily uncomment this line 
        db.create_all() # Creates tables if they don't exist, without deleting data
